...
static MP4Err setEntry( struct MP4SampleToChunkAtom *self, u32 beginningSampleNumber, u32 sampleCount, u32 sampleDescriptionIndex )
{
	stscEntryPtr p;
	MP4Err err;
	...
	TESTMALLOC( p );
	p->firstChunk = 1 + getEntryCount( self );
...
static MP4Err mdatMoved( struct MP4SampleToChunkAtom *self, s32 mdatOffset,
