...
parseMovie( MP4Movie *theMovie, MP4PrivateMovieRecordPtr moov, int openMovieFlags )
{
	...
	for ( finished = 0; !finished; )
	{
		...
		else if ( anAtomPtr->type == MP4FileTypeAtomType)
		{
			//KW: ftyp
			if ( moov->ftyp == NULL )
				moov->ftyp = anAtomPtr;
...
MP4GetMovieMetadata( MP4Movie theMovie, MP4Metadata* outMetadata )
{
	...
	meta = (MP4Metadata) movieAtom->meta;
	/*if ( movieAtom->meta == 0 )
	{
		MP4Handle dataH;
		err	= ISONewHandle(0, &dataH); if (err) goto bail
		err = MP4NewMovieMetadata(movieAtom, &meta, MP4MetadataAtomType, handle ); if (err) goto bail;
		err = movieAtom->addAtom( movieAtom, (MP4AtomPtr) meta ); if (err) goto bail;
	}*/
	*outMetadata = (MP4Metadata) meta;
bail:
	TEST_RETURN( err );
...
MP4NewMovieMetadata( MP4Movie theMovie, MP4Metadata *outMetadata, u32 handlerType, MP4Handle dataReference )
{
   ...
   err = movieAtom->newMetadata( movieAtom, outMetadata, handlerType, dataReference ); if (err) goto bail;
						   ...
						   u8 IsExchangeFile)
{
	...
	if (IsExchangeFile) {
		...
		if (strlen(url)) {
			...
			od->URLString = malloc(sizeof(char) * od->URLStringLength);
		...
		if (strlen(url)) {
			...
			iod->URLString = malloc(sizeof(char) * iod->URLStringLength);
