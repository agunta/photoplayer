...
static MP4Err addOffset(struct MP4ChunkOffsetAtom *self, u64 offset )
{
	MP4Err err;
    err = MP4NoErr;
	self->entryCount++;
	self->offsets = realloc( self->offsets, self->entryCount * sizeof(u64) );
	TESTMALLOC( self->offsets );
	self->offsets[ self->entryCount - 1 ] = (u32) offset;
bail:
	TEST_RETURN( err );
...
MP4Err MP4CreateChunkLargeOffsetAtom( MP4ChunkLargeOffsetAtomPtr *outAtom )
{
	...
	self = (MP4ChunkLargeOffsetAtomPtr) calloc( 1, sizeof(MP4ChunkLargeOffsetAtom) );
	TESTMALLOC( self );
