...
static MP4Err setupNewMedia( struct MP4MediaInformationAtom *self, u32 handlerType, MP4Handle dataH, MP4AtomPtr mdat )
{
	...
	else
	{
		...
		dataEntryAtom->location = calloc( 1, sz );
				    ...
				    MP4Handle decodingOffsetsH, MP4Handle syncSamplesH )
{
	...
	sampleOffset = (u32) dataOffset;
	err = stbl->addSamples( stbl, sampleCount, sampleOffset, durationsH, sizesH, decodingOffsetsH, syncSamplesH ); if (err) goto bail;
...
u32 MP4SampleEntryProtos[] = { MP4MPEGSampleEntryAtomType, MP4VisualSampleEntryAtomType, MP4AudioSampleEntryAtomType, 0 };
...
static MP4Err mdatMoved( struct MP4MediaInformationAtom *self, s32 mdatOffset )
{
	MP4DataInformationAtomPtr   dinf;
	MP4DataReferenceAtomPtr     dref;
	MP4SampleTableAtomPtr       stbl;
	...
	err = stsc->mdatMoved( stsc, mdatOffset, stsd, dref, stco ); if (err) goto bail;
bail:
	TEST_RETURN( err );
	return err;
}
