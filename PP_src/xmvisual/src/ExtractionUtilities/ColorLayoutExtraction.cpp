...
#include <stdio.h>
#include <stdlib.h> // KW: abs(...`)
#include <string.h> //KW: memcpy(...)
#include <math.h>
#include <float.h>
#include "momusys.h"
//#include "inter.h"
...
// end modification for version 3.0
//----------------------------------------------------------------------------
unsigned long ColorLayoutExtractionTool::StartExtracting(int NOfYCoeff, int NOfCCoeff, ImageIO *pImg)
{
	m_NumberOfYCoeff = NOfYCoeff;
	m_NumberOfCCoeff = NOfCCoeff;
    if(m_NumberOfYCoeff<1) m_NumberOfYCoeff=1;
    else if(m_NumberOfYCoeff>64) m_NumberOfYCoeff=64;
    if(m_NumberOfCCoeff<1) m_NumberOfCCoeff=1;
    else if(m_NumberOfCCoeff>64) m_NumberOfCCoeff=64;

    /* check for media*/
    if (!pImg) return (unsigned long)-1;
    MomVop *ImageMedia;
	ImageMedia=pImg->GetImage();
	if(!ImageMedia) return (unsigned long)-1;
    ...
    m_YCoeff[0]=quant_ydc(small_img[0][0]/8)>>1;
    m_CbCoeff[0]=quant_cdc(small_img[1][0]/8);
    m_CrCoeff[0]=quant_cdc(small_img[2][0]/8);
    for(int i=1;i<64;i++){
		m_YCoeff[i]=quant_ac((small_img[0][(zigzag_scan[i])])/2)>>3;
		m_CbCoeff[i]=quant_ac(small_img[1][(zigzag_scan[i])])>>3;
		m_CrCoeff[i]=quant_ac(small_img[2][(zigzag_scan[i])])>>3;
    }
#ifdef DEBUG1
    fprintf(stderr, "CLD (%d %d %d %d %d %d) (%d %d %d) (%d %d %d)\n",
	m_YCoeff[0], m_YCoeff[1], m_YCoeff[2], m_YCoeff[3], m_YCoeff[4], m_YCoeff[5],
	m_CbCoeff[0], m_CbCoeff[1], m_CbCoeff[2], m_CrCoeff[0], m_CrCoeff[1], m_CrCoeff[2]);
#endif
    return 0;
}
