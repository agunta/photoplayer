...
#include <stdio.h>
#include <string.h> // KW 2006: for memcpy
#include <math.h>
#include <float.h>
#include "momusys.h"
#include "inter.h"
...
//---------------------------------------------------------------------------
unsigned long DominantColorExtractionTool::StartExtracting(ImageIO *pImg)
{
	if(!pImg) return -1;
	MomVop *ImageMedia;
...
#ifdef DEBUG
	//std::cerr << "Info(DCE::SE): checks done: " << oldsize << " " << m_imsize << std::endl;
#endif
...
void DominantColorExtractionTool::Extract(unsigned char *quantImageAlpha)
{
  ...
  double  distold=FLT_MAX, distnew, eps=1.0;
  float   aglfct=DSTMIN;
  float   splfct=SPLFCT;
  int     i; //, j, k;

  // maximum number of colours to extract
  m_nummax = 8; //atoi(getpara("MaximumDCNumber", "8"));
  ...
  /* calculate variances */
  bool varpres = true; //(bool)atoi(getpara("VariancePresent", VARPRESDEF));
  if(varpres)
                                        ...
                                        unsigned char *quantAlpha)
{
  ...
  double  ds[DCNMAX][3];  /* variance components */
  double  dists[DCNMAX];
											  ...
											  dc_t *qdesc)
{
	...
	//sprintf(textBINNUMDEF, "%d", BINNUMDEF);
	int bin_number0 = 32; //atoi(getpara("BinNumber0", textBINNUMDEF));
	int bin_number1 = 32; //atoi(getpara("BinNumber1", textBINNUMDEF));
	int bin_number2 = 32; //atoi(getpara("BinNumber2", textBINNUMDEF));
...
// interface for GoFGoP descriptor collection
const dc_t *DominantColorExtractionTool::GetDCDescriptor()
{
	return m_dc;
}
...
//#ifdef DEBUG
//  cerr << "Info(DCF::distance): val: " << val << endl;
//#endif
