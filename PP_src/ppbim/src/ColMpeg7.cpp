...
// Top-level class for collection level metadata
ColMpeg7::ColMpeg7()
{
	m_pLang = NULL;
	m_pChild0 = NULL;
...
bool ColMpeg7::BuildFromXML(DOMNode *node)
{
	...
	if(descNode)
	{
		m_pDescriptionUnit = new ContentCollectionType(true);  // DescriptionUnit of type ContentCollection
...
bool ColMpeg7::Validate()
{
	return true;
}

bool ColMpeg7::WriteBitstream(BitstreamWriter *writer)
{
...
bool ColMpeg7::ReadBitstream(BitstreamReader *reader)
{
	SAFE_DELETE(m_pLang);
	SAFE_DELETE(m_pChild0);
	SAFE_DELETE(m_pDescriptionUnit);
	...
	{
		m_pChild0 = new Mpeg7Type_content();
		RETURN_IFNULL(m_pChild0);
		bret = m_pChild0->ReadBitstream(reader);
	}
	if(bret)
	{
		m_pDescriptionUnit = new ContentCollectionType(true);
		RETURN_IFNULL(m_pDescriptionUnit);
		bret = m_pDescriptionUnit->ReadBitstream(reader);;
	}
	return bret;
}

void ColMpeg7::PrintXML(std::ostream *fout, int nIndentLevel)
{
	if(!fout) return;
	...
	*fout << " xmlns=\"" << MPEG7_COL_SCHEMA_NS << "\" xmlns:xsi=\"" << MPEG7_SCHEMA_XMLNS_XSI;
	*fout << "\" xsi:schemaLocation=\"" << MPEG7_COL_SCHEMA_LOCATION;
	*fout << "\">" << std::endl;
	...
	if(m_pDescriptionUnit)
	{
		...
		*fout << " xsi:type=\"ContentCollectionType\"";
